...
static unsigned short* pFramebuffer = 0;
static unsigned long* pRealBuffer = 0;
static int xres = 0; 
static int yres = 0; 
...
void LCD_L0_SetPixelIndex(int x, int y, int PixelIndex)
{
	...
	*(short*)(pFramebuffer + location) = (short)PixelIndex;
	return; 
}
...
unsigned int LCD_L0_GetPixelIndex(int x, int y) {
	...
	long int location = 0;
    location = x * (bits_per_pixel /8)  + y * xres * (bits_per_pixel / 8);
	PixelIndex = (short) *(pFramebuffer + location);
	return PixelIndex;
}
...
int  LCD_L0_Init(void) {
	xres = LCD_XSIZE;
	yres = LCD_YSIZE;
	bits_per_pixel = LCD_BITSPERPIXEL;
	return 0;
}
