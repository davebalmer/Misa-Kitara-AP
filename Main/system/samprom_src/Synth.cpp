...
{
	std::ifstream ifs(("./presets/" + filename).c_str(), std::ios::binary);
...
//	midi.sendNRPN(SYNTH, 0, 0x37, 0x5E, 127);
		...
		if((control_type == DRAG_X) || (control_type == DRAG_Y))
		{
			if((ae->name != PARAM_MIDI_PITCH) && (ae->name != PARAM_VOICE_PITCH_WHEEL))
			{
				if(val < scaler / 2)
				{
					int max;
					if(!ae->inverse)
						max = ae->drag_center * scaler / 127;
					else
						max = (127-ae->drag_center) * scaler / 127;
				...
				else //if(val >= scaler / 2)
				{
					int min;
					if(!ae->inverse)
						min = ae->drag_center * scaler / 127;
					else
						min = (127-ae->drag_center) * scaler / 127;
